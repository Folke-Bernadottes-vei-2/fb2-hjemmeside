...
function make_content(file) {
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    if (lvl != nlvl) r = r + close_tag_for_lvl(lvl) + open_tag_for_lvl(nlvl);
    if (nlvl >= 0) r = r + line.substring(nlvl, line.length).trim();
  }
...
function menu_lvl(s) {
  if (s.charAt(0) == " " || s.charAt(0) == "\t") return 2;
  return 1;
}
...
function line_to_li(line, lvl, special) {
  ...
  if (s.length > 1) {
    var addr = s[1].trim();
    if (addr != "") {
      if (addr.charAt(0) != "/") addr = "/" + addr;
      if (addr == special) is_special = true;
      if (addr.length > 0) {
        if (addr.charAt(addr.length - 1) != "/") addr = addr + ".html";
        r = '<a href="' + addr + '">' + r + "</a>";
      }
    } else console.log('Empty address field for "' + line + '"');
  }
  var cl = "m" + lvl;
  if (is_special) cl = cl + " highlight hl" + lvl;
...
function menu_change() {
  ...
  } else e.innerHTML = "Laster: " + this.readyState + " / 4";
}
...
function content_change(r) {
  ...
  if (this.readyState === XMLHttpRequest.DONE) {
    e.innerHTML =
      '<div id="innhold">&nbsp;</div>' + make_content_marked(this.response);
...
function ol(x) {
  ...
  e.outerHTML =
    '<header><h1 class="title">Sameiet Folke Bernadottes vei 2</h1></header><div class="oflex"><nav id="nav"><h3>Laster meny</h3></nav><div class="mid">&nbsp;</div><main id="main"><p>Laster innhold</p></main></div><footer><p class="p-footer"><a class="old" href="mailto:styret@fb2.no">styret@fb2.no</a></p></footer>';
  ...
  var y = x;
  if (x.charAt(x.length - 1) == "/") y = x + "index";
